...
Executables shipped by flag_shih_tzu can be used with or without generating the binstubs.
They will work when flag_shih_tzu is installed globally (i.e., `gem install flag_shih_tzu`) and do not require that flag_shih_tzu be in your bundle.
...
However, the rake tasks provided by flag_shih_tzu do require flag_shih_tzu to be added as a development dependency and loaded in your Rakefile.
See the full list of rake tasks in head of Rakefile

**Gemfile**
```ruby
group :development do
  gem "flag_shih_tzu", require: false
end
```
...
- One spec file per class/module. For each class or module under `lib/`, keep all of its unit tests in a single spec file under `spec/` that mirrors the path and file name exactly: `lib/flag_shih_tzu/release_cli.rb` -> `spec/flag_shih_tzu/release_cli_spec.rb`.
...
[🤝conduct]: https://gitlab.com/pboling/flag_shih_tzu/-/blob/main/CODE_OF_CONDUCT.md
